...
public class Potentiometer extends AnalogInput
     ...
     * ------------------------------------------------------
     *
     * @description variable used to determine the state of print to screen option,
     *              default to false
     * @author Bob Brown
     * @written Feb 19, 2011
     *          --------------------------------------------------------
     ...
     * ------------------------------------------------------
     *
     * @description this denotes whether or not the pot is keeping values reversed
     *              (backwards)
     * @author Bob Brown
     * @written Feb 19, 2011
     *          --------------------------------------------------------
    ...
    // -------------------------------------------------------
    /**
     * Create an instance of a Potentiometer class. Creates a analog channel given a
     ...
     *            - the port for the digital input
     * @param maxDegrees
     *            - maximum number of degrees that this potentiometer could travel
     * @author Bob Brown
     * @written Feb 8, 2011 -------------------------------------------------------
     */
    ...
    // -------------------------------------------------------
    /**
     * This function takes the raw value of the potentiometer (out of the max raw
     * units, kTOTAL_RAW_UNITS) and converts it to degrees
     *
     * @method get
    ...
    public int get()
    {
        int retValue = (int) ((this.getValue() / kTOTAL_RAW_UNITS) * this.maxDegreesForPotentiometer);
        // --------------------------------
    ...
    public double get(double range)
    {
        ...
        double retVal = (((2 * this.get()) - this.maxDegreesForPotentiometer) / this.maxDegreesForPotentiometer)
                * (range / 2);
        if (this.isReversed == false)
            {
            // ---------------------------------
    ...
    {
        double retVal = ((this.getValue() / kTOTAL_RAW_UNITS) * (maxRange - minRange)) + minRange;
        if (this.isReversed == false)
            {
            // ---------------------------------
            // if we requested to print out the Normalized
    ...
    /**
     * This function takes the raw value of the potentiometer (out of the max raw
     * units, kTOTAL_RAW_UNITS) and converts it to a given degree range
     *
     * @method get
     * @param degrees
    ...
    public int get(int degrees)
    {
        ...
        retVal = (int) (this.maxDegreesForPotentiometer - ((this.getValue() / kTOTAL_RAW_UNITS) * degrees));
        // ---------------------------------
        // if we requested to print out the Normalized
        // value
        // ---------------------------------
    ...
    // -------------------------------------------------
    /**
     * Gets the location within the range using our max degree value as the range.
     *
     * @return location in the range
     * @author Noah Golmant
     * @written February 9, 2013 ------------------------------------------------
    ...
    {
        // (percent off from middle) * (range of values)
        double retVal = (((2 * this.get()) - this.getMaxDegrees()) / this.getMaxDegrees()) * (this.getMaxDegrees() / 2);
        if (this.isReversed == false)
            {
            // ---------------------------------
            // if we requested to print out the Normalized
    ...
    // -------------------------------------------------------
    /**
     * This function returns weather or not we will print the poteniometer's angle
     ...
     * the potentiometer's angle to drivers station
     *
     * @param newValue
