...
/**
 * One of the more complex drive systems: uses 4 mecanum wheels to allow for
 * lateral movement as well as normal tank controls.
 * 
 * @author Ryan McGee
 * @written 7/21/17
 */
public class MecanumTransmission extends TransmissionBase
     ...
     */
    public MecanumTransmission(MotorController leftRearMotor, MotorController rightRearMotor,
     ...
     * tuning functionality and field-oriented driving.
     * 
     * @param leftFrontMotor
     *            The left front motor controller
     * @param rightFrontMotor
     ...
     */
    public MecanumTransmission(MotorController leftRearMotor, MotorController rightRearMotor,
            MotorController leftFrontMotor, MotorController rightFrontMotor, PIDAnalogGyro gyro)
        {
            super(leftRearMotor, rightRearMotor, leftFrontMotor, rightFrontMotor);
            this.gyro = gyro;
            super.type = TransmissionType.OMNI_DIR;
    ...
    /**
     * Drives the robot with mecanum with raw values, taking into account joystick
     * deadbands and gear ratios.
     * 
     * 
     * @param magnitude
     ...
     * correction PID loop for rotation, if that is enabled.
     * 
     * @param magnitude
    ...
    public void driveRaw(double magnitude, double direction, double rotation)
    {
        ...
        double leftFrontVal = Math.sin(altDirection - (Math.PI / 4.0));
        ...
        super.getMotorController(MotorPosition.LEFT_FRONT).set(inRange((magnitude * leftFrontVal) + altRotation));
        super.getMotorController(MotorPosition.LEFT_REAR).set(inRange((magnitude * leftRearVal) + altRotation));
        super.getMotorController(MotorPosition.RIGHT_FRONT).set(inRange((magnitude * rightFrontVal) - altRotation));
        super.getMotorController(MotorPosition.RIGHT_REAR).set(inRange((magnitude * rightRearVal) - altRotation));
    }
    ...
    /**
     * Sets the percentage added to the magnitude while strafing. This is only used
     * inside the directional deadband, so if it is set to 0, then it will not take
     * affect.
     * 
     * @param value
     ...
     * of degrees that the robot wants to stay at.
     * 
     ...
     * correction. I - Integral - Corrects based on how much time the error has had
     * to build up. The longer the time, the higher the correction. D - Derivative -
     * Overall, this smoothes out the P-loop line, by adding acceleration /
