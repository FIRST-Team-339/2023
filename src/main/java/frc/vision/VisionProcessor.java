...
public class VisionProcessor extends AutoGenVision
    ...
    public VisionProcessor(String ip, CameraModel camera)
        {
            ...
            // stream.
            this.camera = CameraServer.addAxisCamera("axis-camera", ip);
     ...
     *            the brand / model of the camera
     * @param ringlightRelay
     *            - actual relay not just DigitalOutput camera ringlight to pick up
     *            retro-reflective tape: this is the janky fix
     *
     */

    public VisionProcessor(String ip, CameraModel camera, Relay ringlightRelay)
        {
            // Adds the camera to the cscore CameraServer, in order to grab the
            // stream.
            this.camera = CameraServer.addAxisCamera("axis-camera", ip);
    ...
    public VisionProcessor(int usbPort, CameraModel camera)
        {
            // Adds the camera to the cscore CameraServer, in order to grab the
            // stream.
            this.camera = CameraServer.startAutomaticCapture("axis-camera", usbPort);
    ...
    public void processImage()
    {
        ...
        long errorCode = CameraServer.getVideo("axis-camera").grabFrame(image);
        ...
        if (errorCode == 0)
            ...
            System.out.println(CameraServer.getVideo().getError());
            } // end if
    ...
    public void saveImage(ImageType type)
    {
        ...
        // until we get one
        CameraServer.getVideo("axis-camera").grabFrame(tempImage);
